                    ...
                    if row_dict.get('DocDate') and row_dict.get('DocDate').year == datetime.now().year  and row_dict.get('DocDate').month == month:
                        ...
                        if row_dict.get("DebtorName") in ["FOXSEMICON INTEGRATED TECHNOLOGY (SHANGHAI) INC."]:
                            ...
                            for data in record:
                                ...
                                # print(itemListing)
                                if itemListing["ItemCode"] is None or itemListing['FromDocNo'] is None or itemListing["UDF_ActualWeight"] is None:
                                    pass
                                else:
...
monthly_data, total_alimex_sales = salesOrderFetch.salesOrder()

# Process the data
monthly_weights = {}
for record in monthly_data:
    month = record["Date"].strftime("%B")
    item = record["Item Code"]
    weight = record["Weight"]

    if month not in monthly_weights:
        monthly_weights[month] = {}
    if item not in monthly_weights[month]:
        monthly_weights[month][item] = 0

    monthly_weights[month][item] += weight

# Extract months and items
months = list(monthly_weights.keys())
items = set(item for month in monthly_weights.values() for item in month.keys())

# Prepare the data for plotting
weights = np.array([[monthly_weights[month].get(item, 0) for item in items] for month in months])

# Number of months and items
num_months = len(months)
num_items = len(items)

# Set the positions and width for the bars
bar_width = 0.2
bar_positions = np.arange(num_months)

# Create the figure and axis using GridSpec
...
for i, item in enumerate(items):
    bars = ax1.bar(bar_positions + i * bar_width, weights[:, i], bar_width, label=item)
    # Add data labels
    for bar in bars:
        height = bar.get_height()
        if height > 0:
            ax1.text(
                bar.get_x() + bar.get_width() / 2.0,
                height,
                f'{height:,.0f} KG',  # Format the label with commas
                ha='center',
                va='bottom',
                fontsize=12,
...
ax1.set_xticks(bar_positions + bar_width * (num_items - 1) / 2)
ax1.set_xticklabels(months)

# Set the labels and title
ax1.set_xlabel('Months')
ax1.set_ylabel('Weight (KG)')
ax1.set_title('Monthly Value for KGs Sold by Item Code (2024)')
...
# Calculate total weights by item
total_weights_by_item = {item: sum(float(monthly_weights[month].get(item, 0)) for month in months) for item in items}
total_weights = sum(total_weights_by_item.values())

# Prepare data for the pie chart
labels = list(total_weights_by_item.keys())
sizes = list(total_weights_by_item.values())
colors = plt.cm.Paired(range(len(labels)))

# Function to format the pie chart labels
def autopct_format(pct):
    absolute = int(round(pct * total_weights / 100.0))
    return f'{pct:.1f}%\n({absolute:,} KG)'

# Plot the pie chart
ax2.pie(sizes, labels=labels, autopct=autopct_format, startangle=140, colors=colors)
ax2.set_title('Total Weight Sold by Item Code')
